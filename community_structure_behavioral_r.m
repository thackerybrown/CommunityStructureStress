...
s1=randi(200,1,1);
s2=randi([250,400],1,1);
s3=randi([450,580],1,1);
...
%workup test
Screen(mainWindow,'FillRect',backColor);
instructString = 'This phase of the activity is complete. You will now complete a shock workup to calibrate the shock level for the next phase.';
DrawFormattedText(mainWindow,instructString,'center','center',0,0,0,0,2)
Screen('Flip',mainWindow);
rotate=0;
while(1) % wait for space bar
...
%workup test
Screen(mainWindow,'FillRect',backColor);
instructString = 'If the level of shock is at the appropriate level, press the space bar to move on to the next activity.';
DrawFormattedText(mainWindow,instructString,'center','center',0,0,0,0,2)
Screen('Flip',mainWindow);
rotate=0;
while(1) % wait for space bar
...
fprintf(dataFile,'%.4f\ n',chancelevel);
fprintf(dataFile,'\nShock trials\n');
fprintf(dataFile,num2str(s1));
fprintf(dataFile,'\n');   
fprintf(dataFile,num2str(s2));
fprintf(dataFile,'\n');
fprintf(dataFile,num2str(s3));
