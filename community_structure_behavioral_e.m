...
%workup test
Screen(mainWindow,'FillRect',backColor);
instructString = 'If the level of shock is at the appropriate level, press the space bar to move on.';
DrawFormattedText(mainWindow,instructString,'center','center',0,0,0,0,2)
Screen('Flip',mainWindow);
rotate=0;
while(1) % wait for space bar
...
s4=randi([700,1000],1,1);
s5=randi([1100,1350],1,1);
...
for trial=1:length
    ...
    %shock protocol
    if trial == s1 || trial == s2 || trial == s3 || trial == s4 || trial == s5
        trigger_shock_MATLAB()
    end
    % display shape
...
fprintf(dataFile,'\n');
fprintf(dataFile,num2str(s4));
fprintf(dataFile,'\n');
fprintf(dataFile,num2str(s5));
